...
void DS1307::setDateTime(uint16_t year, uint8_t month, uint8_t day,
...
RTCDateTime DS1307::inflateRTCDateTime(uint32_t t) {
    uint32_t original_time = t;
    t -= 946681200;
    ...
    RTCDateTime output;
    output.year = year + 2000;
    output.month = month;
    output.day = day;
    output.hour = hour;
    output.minute = minute;
    output.second = second;
    output.unixtime = original_time;

    return output;
}

void DS1307::setDateTime(uint32_t t) {
    RTCDateTime dateTime = inflateRTCDateTime(t);
...
char *DS1307::dateFormat(char *output_buffer, const char *dateFormat,
                         RTCDateTime dt) {
    output_buffer[0] = 0;
...
void DS1307::addSecons(uint32_t seconds) {
    getDateTime();
...
uint8_t DS1307::bcd2dec(uint8_t bcd) { return ((bcd / 16) * 10) + (bcd % 16); }

uint8_t DS1307::dec2bcd(uint8_t dec) { return ((dec / 10) * 16) + (dec % 10); }
...
long DS1307::time2long(uint16_t days, uint8_t hours, uint8_t minutes,
                       uint8_t seconds) {
...
bool DS1307::isLeapYear(uint16_t year) { return (((year % 4 == 0) && (year % 100 != 0)) || (year % 400 == 0));  }
